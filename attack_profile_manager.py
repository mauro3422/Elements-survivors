...
class AttackProfileManager:
        ...
        try:
            ...
            with open(ruta_completa_config, 'r') as f:
                ...
                if isinstance(data_cargada, dict):
                    ...
                    logger.info(f"Perfiles de ataque cargados desde {self.archivo_config_nombre}")
                    ...
                    if self.nombre_perfil_ataque_activo not in self.perfiles_de_ataque:
                        logger.warning(f"Perfil activo '{self.nombre_perfil_ataque_activo}' no encontrado en el archivo. Intentando usar el predeterminado.")
                        if self.nombre_perfil_inicial_predeterminado in self.perfiles_de_ataque:
                            ...
                            logger.info(f"Usando el primer perfil disponible como activo: {self.nombre_perfil_ataque_activo}")
                        else: # No hay perfiles, forzar creación del default.
                            logger.warning(f"No hay perfiles válidos tras la carga. Forzando creación del perfil por defecto.")
                            self._forzar_creacion_perfil_default_y_guardar() # Esto también setea nombre_perfil_ataque_activo
                else:
                    logger.warning(f"Error: '{self.archivo_config_nombre}' no contiene un diccionario de perfiles. Creando estructura por defecto.")
                    self._forzar_creacion_perfil_default_y_guardar()
        except FileNotFoundError:
            logger.warning(f"Archivo '{self.archivo_config_nombre}' no encontrado. Creando perfil por defecto.")
            self._forzar_creacion_perfil_default_y_guardar()
        except json.JSONDecodeError:
            logger.warning(f"Error JSON en '{self.archivo_config_nombre}'. Creando perfil por defecto.")
            self._forzar_creacion_perfil_default_y_guardar()
    ...
    def _forzar_creacion_perfil_default_y_guardar(self):
        # Esta función se moverá aquí desde Jugador
        logger.info(f"Forzando creación y guardado de perfil por defecto: {self.nombre_perfil_inicial_predeterminado}")
        self.perfiles_de_ataque = {}
    ...
    def guardar_todos_perfiles_ataque(self):
        # Esta función se moverá aquí desde Jugador
        logger.info(f"Guardando todos los perfiles de ataque en: {self.archivo_config_nombre}")
        try:
            ruta_completa_config = os.path.join(self.ruta_base_proyecto, self.archivo_config_nombre)
            with open(ruta_completa_config, 'w') as f:
                json.dump(self.perfiles_de_ataque, f, indent=4)
            logger.info(f"Todos los perfiles de ataque guardados en {self.archivo_config_nombre}")
        except Exception as e:
    ...
    def seleccionar_perfil_ataque(self, nombre_perfil_solicitado):
        ...
        logger.debug(f"Intento de seleccionar perfil de ataque: {nombre_perfil_solicitado}")
        ...
        if nombre_perfil_solicitado in self.perfiles_de_ataque and isinstance(self.perfiles_de_ataque[nombre_perfil_solicitado], dict):
            ...
            if self._num_segmentos_barrido_activo > 0 and duracion_total > 0:
                ...
                self._duracion_segmento_barrido_activo = 0
                if self._num_segmentos_barrido_activo == 0: logger.warning("Num segmentos es 0, duracion_segmento será 0.")

            logger.info(f"Perfil activo cambiado a: '{self.nombre_perfil_ataque_activo}'. Dur seg: {self._duracion_segmento_barrido_activo:.2f}ms. Num_seg: {self._num_segmentos_barrido_activo}. Dur_total: {duracion_total}")
            return True
        else:
            ...
            if nombre_perfil_solicitado != self.nombre_perfil_inicial_predeterminado and \
               ...
               isinstance(self.perfiles_de_ataque.get(self.nombre_perfil_inicial_predeterminado), dict):
                logger.info(f"Fallback al perfil por defecto: {self.nombre_perfil_inicial_predeterminado}")
            ...
            if not (self.nombre_perfil_ataque_activo in self.perfiles_de_ataque and isinstance(self.perfiles_de_ataque[self.nombre_perfil_ataque_activo], dict)):
                logger.warning("Ningún perfil válido disponible. Forzando recreación del perfil por defecto y seleccionándolo.")
            ...
            logger.critical(f"CRÍTICO: No se pudo seleccionar un perfil válido ('{nombre_perfil_solicitado}' intentado) incluso después de intentar fallbacks y forzar creación. Perfil activo actual: '{self.nombre_perfil_ataque_activo}'")
            return False
    ...
    def get_parametro_ataque_activo(self, nombre_parametro, valor_defecto=None):
        ...
        logger.warning(f"Perfil activo '{self.nombre_perfil_ataque_activo}' no encontrado o no es dict al obtener '{nombre_parametro}'. Usando valor por defecto.")
    ...
    def set_parametro_ataque_activo(self, nombre_parametro, valor):
        ...
        if isinstance(perfil, dict):
            ...
            logger.debug(f"Parámetro '{nombre_parametro}' seteado a '{valor}' en perfil '{self.nombre_perfil_ataque_activo}'")
            ...
            if nombre_parametro == "duracion_total_ms" or nombre_parametro == "plantilla_angulos_grados":
                logger.info(f"Parámetro clave '{nombre_parametro}' modificado. Reseleccionando perfil para recalcular derivados.")
                self.seleccionar_perfil_ataque(self.nombre_perfil_ataque_activo) # Recalcular duraciones de segmento, etc.
        else:
            logger.warning(f"Advertencia (set_parametro): No se pudo setear '{nombre_parametro}' en perfil '{self.nombre_perfil_ataque_activo}'. Perfil no encontrado o no es dict.")
    ...
    def modificar_ataque_extension(self, cantidad):
        ...
        nuevo = max(1, float(actual) + float(cantidad))
    ...
    def modificar_duracion_ataque_total(self, cantidad):
        ...
        nuevo = max(50, float(actual) + float(cantidad)) # Duración mínima de 50ms
        self.set_parametro_ataque_activo("duracion_total_ms", nuevo)
        # set_parametro_ataque_activo ya llama a seleccionar_perfil_ataque si es duracion_total_ms
        logger.debug(f"Perfil '{self.nombre_perfil_ataque_activo}' - duracion_total_ms: {nuevo}. Segmentos y duración recalculados.")
