...
class EntidadBase(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, asset_manager_instance,
        ...
        if dict_animaciones_config:
            ...
            self.image.fill(settings.ROJO_ERROR_ASSET) # Usar la constante directamente
        ...
        if self.animaciones.get(self.estado_animacion) and self.animaciones[self.estado_animacion]:
            ...
            self.image = pygame.Surface((32, 32)); self.image.fill(settings.ROJO_ERROR_ASSET)
    ...
    def actualizar_animacion(self, delta_time_ms=None):
        ...
        if settings.MODO_DEBUG_LOGS and settings.LOG_CATEGORIAS.get("log_animacion", False):
            logger.debug(f"{self.nombre_log_entidad} INICIO EntidadBase.actualizar_animacion. Estado: {self.estado_animacion}, delta_time_ms: {delta_time_ms}", extra={"categoria_log": "log_animacion"})
    ...
    def recibir_dano(self, cantidad, tipo_dano="generico"):
        ...
        if "Jugador" in self.nombre_entidad_tipo:
            ...
            categoria_log_vida = "log_enemigo"
        ...
        # Para logger.info, la visibilidad depende de que la categoría esté activa y el nivel general del logger.
        # No se necesita MODO_DEBUG_LOGS para INFO si la categoría está activa.
        if settings.LOG_CATEGORIAS.get(categoria_log_vida, False): 
        ...
        if self.vida_actual <= 0:
            ...
            if hasattr(self, 'cooldown_invulnerabilidad_ms') and self.cooldown_invulnerabilidad_ms > 0:
